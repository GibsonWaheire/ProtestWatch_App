...
function Events() {
  ...
  return (
    <div className="p-6">
  <h2 className="text-3xl font-bold italic text-center text-black mb-4">Events</h2>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
    {events.map((event) => (
      <div key={event.id} className="border p-4 rounded-lg bg-white shadow">
        ...
        <p className="text-blue-500 text-center"> {event.location}</p>
        <p className="text-gray-600 text-center"> {event.date}</p>
        <p className="text-gray-600 text-center"> Status: {event.status}</p>
